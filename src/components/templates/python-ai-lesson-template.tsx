...
import { isPythonAiLessonUnlocked } from "@/domain/learning/python-ai/lesson-unlock";
...
type PythonAiLessonTemplateProps = {
  params: Promise<{ slug: string }>;
};

export async function PythonAiLessonTemplate({
  params,
}: PythonAiLessonTemplateProps) {
  ...
  const unlocked = isPythonAiLessonUnlocked(slug, {
  ...
  if (!unlocked) {
    return (
      <article className="flex flex-col gap-4">
        <Link href="/learn/python-ai" className="text-sm underline">
          {t("backToModule")}
        </Link>
  ...
  return (
    <article className="flex flex-col gap-6">
      ...
      <LessonCompleteButton
        ...
        progressBasePath="/learn/python-ai"
        i18nNamespace="pythonAi"
      />
      ...
      >
        {prev ? (
          <Link
            href={`/learn/python-ai/${prev.slug}`}
        ...
        {next ? (
          <Link
            href={`/learn/python-ai/${next.slug}`}
            className="text-sm font-medium underline"
          >
