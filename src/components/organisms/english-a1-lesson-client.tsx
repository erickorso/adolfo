...
import {
  EnglishA1LessonsNotesSidebar,
  type CompletedLessonNote,
} from "@/components/organisms/english-a1-lessons-notes-sidebar";
...
type EnglishA1LessonClientProps = {
  ...
  tipTitle: string;
  grammarTitle: string;
  completedLessons: CompletedLessonNote[];
};
...
export function EnglishA1LessonClient({
  ...
  return (
    <EnglishA1VocabProvider>
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(220px,260px)_minmax(0,1fr)_minmax(240px,280px)] lg:items-start">
        <EnglishA1DictionarySidebar />
        ...
        <EnglishA1LessonsNotesSidebar lessons={completedLessons} />
      </div>
    </EnglishA1VocabProvider>
  );
}
