...
type PythonAiModuleProgressProps = {
  progress: ModuleProgressVM;
};

export async function PythonAiModuleProgress({
  progress,
}: PythonAiModuleProgressProps) {
  const t = await getTranslations("pythonAi");

  return (
    <section
      className="rounded-lg border border-border bg-card p-6"
      aria-labelledby="python-ai-progress-heading"
    >
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <h2
          id="python-ai-progress-heading"
        ...
        {progress.isLoggedIn ? (
          ...
          <Link
            href="/login?callbackUrl=/learn/python-ai"
