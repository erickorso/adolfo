# Job boards — remoto (fuera de LinkedIn/Indeed)

> Lista de nichos recomendados (video Michael Lykins, jun 2026).  
> Prioridad para tu perfil: **Senior React/Next.js**, remoto, EU/Global.

| # | Sitio | URL | Notas |
|---|---|---|---|
| 1 | GitHub Jobs | https://jobs.github.com | ⚠️ **Cerrado** (2017). Usar [GitHub Careers](https://github.com/about/careers) o búsqueda en repos |
| 2 | Career Hound | https://careerhound.io | Agregador remoto |
| 3 | EuropeRemotely | https://europeremotely.com | **EU remoto** — buen fit desde España |
| 4 | Jobspresso | https://jobspresso.co | Curated remote |
| 5 | Dynamite Jobs | https://dynamitejobs.com | Remote-first |
| 6 | SkipTheDrive | https://skipthedrive.com | Clásico WFH |
| 7 | Pangian | https://pangian.com | Comunidad + jobs remoto |
| 8 | Outsourcely | https://outsourcely.com | Freelance / long-term remote |
| 9 | Virtual Vocations | https://virtualvocations.com | Remoto verificado |
| 10 | CloudPeeps | https://cloudpeeps.com | Freelance / contract |
| 11 | Remotive | https://jobs.remotive.io | **Muy usado** — filtrar Dev / React |

## Extra (del mismo video)

| Sitio | URL | Notas |
|---|---|---|
| RezPass | https://rezpass.com | Herramienta / apply helper (revisar antes de pagar) |

## Pendiente — explorar juntos (2026-07-03)

> Sitios anotados desde video/lista para **sesión de búsqueda posterior**. Prioridad: Senior React/Next, remoto, EU/Global.

| Sitio | URL | Fit / notas |
|---|---|---|
| **Wellfound** | https://wellfound.com | Startups + tech remoto; antes AngelList Talent. Buen para roles US/LATAM startup |
| **Career Hound** | https://careerhound.io | Agregador remoto — ya en lista principal (#2) |
| **We Work Remotely** | https://weworkremotely.com | Clásico remote-first; filtrar Programming / Front-End |
| **Built In** | https://builtin.com | Tech hubs (US + algunas ciudades EU); muchas empresas medianas/grandes |
| **Remote 100k** | https://remote100k.com | Roles remotos **$100k+ USD** — senior/lead; revisar si aceptan España/LATAM |
| **Hacker News Jobs** | https://news.ycombinator.com/jobs | Startups **YC** — calidad alta · ingesta `hackernews` |
| **Build Your Own X** | https://github.com/codecrafters-io/build-your-own-x | Tutoriales “desde cero” — ver [`build-your-own-x-resources.md`](./build-your-own-x-resources.md) |

**Estado:** `todo` — no aplicado aún en esta tanda.

---

## Otros que ya tenés en la estrategia

| Sitio | URL |
|---|---|
| We Work Remotely | https://weworkremotely.com |
| RemoteOK | https://remoteok.com |
| Wellfound | https://wellfound.com |
| Otta | https://otta.com |

## Rutina sugerida (2×/semana)

1. **EuropeRemotely** + **Remotive** + **Dynamite Jobs** — aplicar 2–3 roles
2. Anotar cada apply en [`progress.md`](./progress.md)
3. Alertas RSS/email donde existan

## Búsquedas útiles

```
React
Next.js
Frontend
Senior Frontend
TypeScript
Frontend Lead
```

---

*Fuente: lista compartida jun 2026 · actualizado 2026-07-03 (Wellfound, Career Hound, WWR, Built In, Remote 100k)*
