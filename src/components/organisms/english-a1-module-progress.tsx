...
type EnglishA1ModuleProgressProps = {
  progress: ModuleProgressVM;
};

export async function EnglishA1ModuleProgress({
  progress,
}: EnglishA1ModuleProgressProps) {
  const t = await getTranslations("englishA1");

  return (
    <section
      className="rounded-lg border border-border bg-card p-6"
      aria-labelledby="english-a1-progress-heading"
    >
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <h2
          id="english-a1-progress-heading"
        ...
        {progress.isLoggedIn ? (
          ...
          <Link
            href="/login?callbackUrl=/learn/english-a1"
