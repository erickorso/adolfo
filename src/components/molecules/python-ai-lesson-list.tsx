...
type PythonAiLessonListProps = {
  completedSlugs: string[];
  isLoggedIn: boolean;
};

export async function PythonAiLessonList({
  completedSlugs,
  isLoggedIn,
}: PythonAiLessonListProps) {
  ...
  return (
    <ol className="flex flex-col gap-3" aria-label={t("lessonListLabel")}>
      {PYTHON_AI_LESSONS.map((lesson) => {
        const unlocked = isPythonAiLessonUnlocked(lesson.slug, unlockProgress);
          ...
          >
            <div className="flex flex-col gap-1">
              ...
              <div className="flex items-center gap-2">
                ...
                {unlocked ? (
                  <Link
                    href={`/learn/python-ai/${lesson.slug}`}
